...
public function getCustomersWithDues($cid)
{
    ...
    // Main query – now includes additional payments directly via LEFT JOIN
    $customersWithDues = DB::table('sales_clients as sc')
        ->join('sales_bills as sb', 'sc.id', '=', 'sb.scid')
        ...
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
            DB::raw("TO_CHAR(SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100) - COALESCE(sb.absolute_discount, 0)), 'FM999999999.00') as total_purchase"),
            DB::raw("TO_CHAR(COALESCE(SUM(sb.paid_amount), 0) + COALESCE(SUM(cp.paid_amount), 0), 'FM999999999.00') as total_paid"),
            DB::raw("TO_CHAR(
                SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100) - COALESCE(sb.absolute_discount, 0) - COALESCE(sb.paid_amount, 0))
        ...
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw(
            "SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100) - COALESCE(sb.absolute_discount, 0) - COALESCE(sb.paid_amount, 0)) - COALESCE(SUM(cp.paid_amount), 0) > 0"
        )
        ->get();
