...
public function getCustomersWithDues($cid)
{
    ...
    // Subquery: Calculate and ROUND each bill's gross to 2 decimals (critical!)
    $billGrossRounded = DB::table('sales_items')
        ->select(
            'bid',
            DB::raw('ROUND(SUM(s_price * quantity * (1 - dis / 100) * (1 + gst / 100)), 2) as gross_rounded')
        )
        ->groupBy('bid');

    // Subquery: Total paid per bill (no change needed, payments are already in 2 decimals)
    ...
    $customersWithDues = DB::table('sales_clients as sc')
        ->join('sales_bills as sb', 'sc.id', '=', 'sb.scid')
        ->leftJoinSub($billGrossRounded, 'bg', function ($join) {
        ...
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
            DB::raw('SUM(COALESCE(bg.gross_rounded, 0) - sb.absolute_discount) as raw_purchase'),
            DB::raw('SUM(COALESCE(bp.total_paid, 0)) as raw_paid')
        )
        ->where('sc.cid', $cid)
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw('SUM(COALESCE(bg.gross_rounded, 0) - sb.absolute_discount - COALESCE(bp.total_paid, 0)) > 0.01')
        ->get();

    // Final formatting in PHP
    $customersWithDues = $customersWithDues->map(function ($customer) {
        ...
        $due      = max(0, $purchase - $paid); // prevent negative due to tiny errors
