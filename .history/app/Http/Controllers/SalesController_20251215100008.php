...
public function getCustomersWithDues($cid)
{
    ...
    // This is the new part that fixes everything – counts all your test payments
    $additionalPayments = DB::table('customer_bill_payments')
        ->select('customer_id', DB::raw('COALESCE(SUM(paid_amount), 0) as extra_paid'))
        ->groupBy('customer_id');
    ...
    $customersWithDues = DB::table('sales_clients as sc')
        ...
        ->leftJoinSub($additionalPayments, 'ap', function ($join) {
            $join->on('sc.id', '=', 'ap.customer_id');
        })
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
            DB::raw("TO_CHAR(SUM(bt.item_total - sb.absolute_discount), 'FM999999999.00') as total_purchase"),
            // Fixed line 1: Now adds all your test payments
            DB::raw("TO_CHAR(SUM(sb.paid_amount) + COALESCE(ap.extra_paid, 0), 'FM999999999.00') as total_paid"),
            // Fixed line 2: Now subtracts all your test payments
            DB::raw("TO_CHAR(SUM(bt.item_total - sb.absolute_discount - sb.paid_amount) - COALESCE(ap.extra_paid, 0), 'FM999999999.00') as total_due")
        )
        ->where('sc.cid', $cid)
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw('SUM(bt.item_total - sb.absolute_discount - sb.paid_amount) - COALESCE(ap.extra_paid, 0) > 0')
        ->get();
