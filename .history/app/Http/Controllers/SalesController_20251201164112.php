...
public function b2cSalesReport(Request $request)
{
    ...
    // THIS IS THE BULLETPROOF QUERY — 100% ACCURATE
    $data = DB::table('sales_items as si')
        ->join('sales_bills as sb', 'si.bid', '=', 'sb.id')
        ->join('products as p', 'si.pid', '=', 'p.id')
        ->join('sales_clients as sc', 'sb.scid', '=', 'sc.id')
        ->where('sb.cid', $cid)  // ← CRITICAL: Use sb.cid (bill belongs to company)
        ->where(function ($q) {
            $q->whereNull('sc.gst_no')
              ->orWhere('sc.gst_no', '=', '')
              ->orWhere('sc.gst_no', 'LIKE', '""');
        })
        ->where('sb.updated_at', '>=', $startDate)
        ->where('sb.updated_at', '<=', $endDate)
        ->select(
            'p.id as product_id',
            'p.name as item_name',
            'p.hscode',
            ...
            DB::raw('
                ROUND(
                    SUM(
                        si.quantity * si.s_price * (1 - COALESCE(si.dis, 0) / 100)
                    ), 2
                ) as total_taxable
            '),
            DB::raw('
                ROUND(
                    SUM(
                        si.quantity * si.s_price * (1 - COALESCE(si.dis, 0) / 100) * (si.gst / 100)
                    ), 2
                ) as total_gst
            ')
        )
    ...
    $report = $data->map(function ($row) {
        ...
        return [
            ...
            'gst'           => (float)$row->total_gst,
            'total'         => round((float)$row->total_taxable + (float)$row->total_gst, 2),
        ];
    });

    // EXCEL GENERATION — PERFECT
    return response()->streamDownload(function () use ($report, $request) {
        ...
        $headers = ['Item Name', 'HSN Code', 'CGST %', 'SGST %', 'IGST %', 'Taxable Value', 'GST Amount', 'Total Amount'];
        ...
        $sheet->getStyle("E$lastRow:H$lastRow")
            ->getFont()->setBold(true)
            ->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
            ->getStartColor()->setARGB('FFF4C4C4');
    ...
    }, 'B2C_Sales_Report_' . $request->start_date . '_to_' . $request->end_date . '.xlsx');
}
}