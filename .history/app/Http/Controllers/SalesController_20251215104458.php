...
public function getCustomersWithDues($cid)
{
    ...
    $customersWithDues = DB::table('sales_clients as sc')
        ...
        ->leftJoinSub($billGross, 'bg', fn($join) => $join->on('sb.id', '=', 'bg.bid'))
        ->leftJoinSub($billPayments, 'bp', fn($join) => $join->on('sb.id', '=', 'bp.bill_id'))
        ->select(
        ...
        ->havingRaw('SUM(COALESCE(bg.gross_amount, 0) - sb.absolute_discount - COALESCE(bp.total_paid, 0)) > 0.01') // small tolerance
        ->orderByRaw('total_due DESC')
        ->get();
