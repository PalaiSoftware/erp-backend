...
public function getCustomerDues(Request $request, $customer_id)
{
    ...
    // THIS LINE FIXES EVERYTHING – reads amount from GET, POST, JSON, form-data
    ...
    // RE-CALCULATE LEDGER (100% correct)
    ...
    $totalBilled = 0;
    $totalPaid   = $bills->sum('initial_paid') + $laterPaid;
    $remaining   = $laterPaid;
    $ledger      = [];
    ...
    foreach ($bills as $b) {
        ...
        $ledger[] = [
            ...
            'due'       => number_format(max(0, $billTotal - $paid), 2),
            'status'    => ($billTotal - $paid) <= 0.01 ? 'Paid' : 'Due'
        ];
    }
