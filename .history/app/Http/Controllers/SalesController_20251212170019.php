...
public function getCustomersWithDues($cid)
{
    ...
    foreach ($customers as $c) {
        ...
        // Get all bills with gross amount (same as ledger)
        ...
        // Total paid from both sources
        $totalPaid = DB::table('sales_bills')->where('scid', $customerId)->sum('paid_amount') ?? 0;
        $totalPaid += DB::table('customer_bill_payments')->where('customer_id', $customerId)->sum('paid_amount') ?? 0;

        // Apply FIFO exactly like ledger
