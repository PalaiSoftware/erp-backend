...
public function b2cSalesReport(Request $request)
{
    ...
    $report = DB::table('sales_items as si')
        ...
        ->leftJoin('users as u', 'sb.uid', '=', 'u.id')  // ← THIS WAS BROKEN BEFORE
        ->where('u.cid', $cid)
        ->where(function ($q) {
        ...
        ->select(
            ...
            'p.serial_number',
            'si.gst as gst_rate',
            // Detect if CGST/SGST or IGST was applied
            DB::raw('MAX(COALESCE(si.cgst, 0)) as max_cgst'),
            DB::raw('MAX(COALESCE(si.sgst, 0)) as max_sgst'),
            DB::raw('MAX(COALESCE(si.igst, 0)) as max_igst'),
    ...
    $finalReport = $report->map(function ($row) {
        ...
        $hasCGSTSGST = ($row->max_cgst > 0 || $row->max_sgst > 0);
        $hasIGST     = ($row->max_igst > 0);

        return (object)[
            'item_name'      => $row->item_name . ($row->serial_number ? " ({$row->serial_number})" : ''),
            'hscode'         => $row->hscode ?: 'N/A',
            'cgst_rate'      => $hasCGSTSGST ? number_format($gstRate / 2, 1) : '0.0',
            'sgst_rate'      => $hasCGSTSGST ? number_format($gstRate / 2, 1) : '0.0',
            'igst_rate'      => $hasIGST ? number_format($gstRate, 1) : '0.0',
