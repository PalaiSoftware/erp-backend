...
public function getCustomersWithDues($cid)
{
    ...
    // Calculate gross amount per bill from items (with discount & GST)
    $billGross = DB::table('sales_items')
        ->select('bid', DB::raw('SUM(s_price * quantity * (1 - dis / 100) * (1 + gst / 100)) as gross_amount'))
        ->groupBy('bid');

    // Total paid per bill from the NEW payment table
    $billPayments = DB::table('customer_bill_payments')
        ->select('bill_id', DB::raw('COALESCE(SUM(paid_amount), 0) as total_paid'))
        ->groupBy('bill_id');
    ...
    $customersWithDues = DB::table('sales_clients as sc')
        ...
        ->select(
            ...
            'sc.name as customer_name',
            DB::raw("TO_CHAR(SUM(COALESCE(bg.gross_amount, 0) - sb.absolute_discount), 'FM999999999.00') as total_purchase"),
            DB::raw("TO_CHAR(SUM(COALESCE(bp.total_paid, 0)), 'FM999999999.00') as total_paid"),
            DB::raw("TO_CHAR(SUM(COALESCE(bg.gross_amount, 0) - sb.absolute_discount - COALESCE(bp.total_paid, 0)), 'FM999999999.00') as total_due")
        )
        ->where('sc.cid', $cid)
