...
public function b2cSalesReport(Request $request)
{
    ...
    return response()->streamDownload(function () use ($finalReport, $startDate, $endDate) {
        ...
        $sheet->setCellValue("F$lastRow", $finalReport->sum('taxable_amount'));
        $sheet->setCellValue("G$lastRow", $finalReport->sum('total_gst'));
        $sheet->setCellValue("H$lastRow", $finalReport->sum('total_amount'));
