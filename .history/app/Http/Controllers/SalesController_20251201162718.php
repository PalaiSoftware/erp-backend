...
public function b2cSalesReport(Request $request)
{
    ...
    // GROUP BY PRODUCT + GST RATE → Same product with 12% and 18% will be separate rows
    $data = DB::table('sales_items as si')
        ...
        ->whereBetween('sc.gst_no', ''))
        ->whereBetween('sb.updated_at', [$startDate, $endDate])
        ->select(
            'p.name as item_name',
            'p.hscode',
            'si.gst as gst_rate',
            DB::raw('ROUND(SUM(si.quantity * si.s_price * (1 - COALESCE(si.dis, 0)/100)), 2) as total_taxable'),
            DB::raw('ROUND(SUM(si.quantity * si.s_price * (1 - COALESCE(si.dis, 0)/100) * (si.gst / 100)), 2) as total_gst_amount')
        )
        ->groupBy('p.id', 'p.name', 'p.hscode', 'si.gst')
        ->orderBy('p.name')
        ->get();
    ...
    if ($data->isEmpty()) {
        return response()->json(['message' => 'No B2C sales found'], 404);
    }

    $report = $data->map(function ($row) {
        $rate = (float) $row->gst_rate;
        $taxable = (float) $row->total_taxable;
        $gst = (float) $row->total_gst_amount;
    ...
    return response()->streamDownload(function () use ($report, $startDate, $endDate) {
        ...
        $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(16)->getAlignment()->setHorizontal('center');

        $sheet->setCellValue('A2', "Period: {$startDate->format('d-m-Y')} to {$endDate->format('d-m-Y')}");
        $sheet->mergeCells('A2:H2');

        // Headers
        $headers = ['Product Name', 'HSN/SAC', 'CGST %', 'SGST %', 'IGST %', 'Taxable Value', 'GST Amount', 'Total Amount'];
        ...
        $sheet->getStyle("E$lastRow:H$lastRow")->getFill()
            ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
