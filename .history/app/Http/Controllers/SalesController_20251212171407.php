...
public function getCustomersWithDues($cid)
{
    ...
    foreach ($customers as $c) {
        ...
        // === EXACT SAME LOGIC AS YOUR LEDGER (THIS IS THE TRUTH) ===
        $bills = DB::table('sales_bills as sb')
            ...
            ->groupBy('sb.id', 'sb.absolute_discount')
            ->orderBy('sb.updated_at', 'asc')
            ->get();

        // Total paid from sales_bills (initial payment)
        $totalPaid = DB::table('sales_bills')
            ->where('scid', $customerId)
            ->sum('paid_amount') ?? 0;

        // Total paid from customer_bill_payments (later payments) — JOIN with bill to get correct customer
        $totalPaid += DB::table('customer_bill_payments as cp')
            ->join('sales_bills as sb', 'cp.bill_id', '=', 'sb.id')
            ->where('sb.scid', $customerId)
            ->sum('cp.paid_amount') ?? 0;

        // Apply FIFO (same as ledger)
        ...
        $currentDue = max(0, $totalBilled - $totalPaid);

        if ($currentDue > 0.01) {
            $result[] = [
                'customer_id'    => $customerId,
                'customer_name'  => $c->name,
                'total_purchase' => number_format($totalBilled, 2),
