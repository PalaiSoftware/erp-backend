...
// ADD THIS HELPER FUNCTION AT THE TOP OF YOUR CONTROLLER (inside class)
...
public function getCustomersWithDues($cid)
{
    ...
    $customers = DB::table('sales_clients')
        ...
        ->whereExists(function ($query) {
            $query->select(DB::raw(1))
                  ->from('sales_bills')
                  ->whereColumn('sales_clients.id', 'sales_bills.scid');
        })
        ->select('id', 'name')
        ->get();
    ...
    foreach ($customers as $c) {
        ...
        $billed = DB::table('sales_bills as sb')
            ...
            ->where('sb.scid', $cid)
            ->selectRaw('COALESCE(SUM(si.quantity * si.s_price * (1 - COALESCE(si.dis,0)/100) * (1 + COALESCE(si.gst,0)/100)) - SUM(sb.absolute_discount), 0)')
            ->value(DB::raw('1')) ?? 0;

        // Total Paid = Initial + Later payments via bill_id
        $initial = DB::table('sales_bills')->where('scid', $cid)->sum('paid_amount') ?? 0;
        $later = DB::table('customer_bill_payments as cp')
            ->join('sales_bills as sb', 'cp.bill_id', '=', 'sb.id')
            ->where('sb.scid', $cid)
            ->sum('cp.paid_amount') ?? 0;

        $totalPaid = $initial + $later;
        $due = max(0, $billed - $totalPaid);
