...
public function b2cSalesReport(Request $request)
{
    ...
    if (!in_array($user->rid, [1, 2, 3])) return response()->json(['message' => 'Forbidden'], 403);
    ...
    $finalReport = $report->map(function ($row) {
        ...
        return (object)[
            ...
            'cgst_rate'      => $gstRate > 0 ? $gstRate / 2 : 0,
            'sgst_rate'      => $gstRate > 0 ? $gstRate / 2 : 0,
            'igst_rate'      => 0,
    ...
    // THIS LINE MAKES IT DOWNLOAD TO YOUR LOCAL PC INSTANTLY
    return Excel::download(
        new B2CSalesReportExport($finalReport, $dateRange),
        'B2C_Sales_Report_' . now()->format('d_M_Y_h_i_A') . '.xlsx'
    );
}
