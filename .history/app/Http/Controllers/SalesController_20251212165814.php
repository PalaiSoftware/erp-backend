...
public function getCustomersWithDues($cid)
{
    ...
    // Get all customers who have sales
    ...
    foreach ($customers as $c) {
        ...
        // === EXACT SAME LOGIC AS YOUR LEDGER (100% ACCURATE) ===
        ...
        foreach ($bills as $bill) {
            $billTotal = $bill->gross_amount - ($bill->absolute_discount ?? 0);
            $totalBilled += $billTotal;
        }

        // Total Paid = Initial + Later payments
        $initialPaid = DB::table('sales_bills')->where('scid', $customerId)->sum('paid_amount') ?? 0;
        $laterPaid = DB::table('customer_bill_payments')->where('customer_id', $customerId)->sum('paid_amount') ?? 0;
        $totalPaid = $initialPaid + $laterPaid;
