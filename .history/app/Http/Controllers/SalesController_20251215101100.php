...
public function getCustomersWithDues($cid)
{
    ...
    $customersWithDues = DB::table('sales_clients as sc')
        ...
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
            // Correct total billed: sum of (item totals - absolute_discount per bill)
            DB::raw("TO_CHAR(
                COALESCE(SUM(
                    si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100)
                ), 0) 
                - COALESCE(SUM(sb.absolute_discount), 0)
            , 'FM999999999.00') as total_purchase"),
            // Total paid: initial paid + all later payments
            DB::raw("TO_CHAR(
                COALESCE(SUM(sb.paid_amount), 0) + COALESCE(SUM(cp.paid_amount), 0)
            , 'FM999999999.00') as total_paid"),
            // Total due: total_purchase - total_paid
            DB::raw("TO_CHAR(
                GREATEST(0,
                    COALESCE(SUM(
                        si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100)
        ...
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw(
            "COALESCE(SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100)), 0)
             - COALESCE(SUM(sb.absolute_discount), 0)
             - COALESCE(SUM(sb.paid_amount), 0)
