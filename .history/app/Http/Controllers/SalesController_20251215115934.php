...
public function getCustomersWithDues($cid)
{
    ...
    // Bill gross (precise, no rounding yet)
    $billGross = DB::table('sales_items')
        ->select('bid', DB::raw('SUM(s_price * quantity * (1 - dis / 100) * (1 + gst / 100)) as gross_amount'))
        ->groupBy('bid');

    // Payments from new table (precise)
    $billPayments = DB::table('customer_bill_payments')
        ->select('bill_id', DB::raw('SUM(paid_amount) as total_paid'))
        ->groupBy('bill_id');

    $customersWithDues = DB::table('sales_clients as sc')
        ->join('sales_bills as sb', 'sc.id', '=', 'sb.scid')
        ->leftJoinSub($billGross, 'bg', function ($join) {
            $join->on('sb.id', '=', 'bg.bid');
        })
        ->leftJoinSub($billPayments, 'bp', function ($join) {
            $join->on('sb.id', '=', 'bp.bill_id');
        })
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
            // Calculate raw sums first (higher precision)
            DB::raw('SUM(COALESCE(bg.gross_amount, 0) - sb.absolute_discount) as raw_purchase'),
            DB::raw('SUM(COALESCE(bp.total_paid, 0)) as raw_paid')
        )
        ->where('sc.cid', $cid)
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw('SUM(COALESCE(bg.gross_amount, 0) - sb.absolute_discount - COALESCE(bp.total_paid, 0)) > 0.01')
        ->orderByRaw('SUM(COALESCE(bg.gross_amount, 0) - sb.absolute_discount - COALESCE(bp.total_paid, 0)) DESC')
        ->get();

    // Now format in PHP for consistent 2-decimal rounding
    $customersWithDues = $customersWithDues->map(function ($customer) {
        $purchase = round((float)$customer->raw_purchase, 2);
        $paid     = round((float)$customer->raw_paid, 2);
        $due      = $purchase - $paid;

        // Ensure due doesn't go negative due to tiny float errors
        $due = max(0, $due);

        $customer->total_purchase = number_format($purchase, 2, '.', ',');
        $customer->total_paid     = number_format($paid, 2, '.', ',');
        $customer->total_due      = number_format($due, 2, '.', ',');

        unset($customer->raw_purchase, $customer->raw_paid);
