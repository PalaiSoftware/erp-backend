...
public function getCustomersWithDues($cid)
{
    ...
    // Subquery: Calculate actual bill total (after item discount + GST, before absolute discount)
    $billTotals = DB::table('sales_items')
        ->select('bid', DB::raw('SUM(s_price * quantity * (1 - dis / 100) * (1 + gst / 100)) as gross_amount'))
        ->groupBy('bid');

    // Subquery: Actual payments per bill from the new payment table
    ...
    $customersWithDues = DB::table('sales_clients as sc')
        ...
        ->leftJoinSub($billTotals, 'bt', function ($join) {
        ...
        ->select(
            ...
            'sc.name as customer_name',
            DB::raw("TO_CHAR(SUM(COALESCE(bt.gross_amount, 0) - sb.absolute_discount), 'FM999999999.00') as total_purchase"),
            DB::raw("TO_CHAR(SUM(COALESCE(bp.total_paid, 0)), 'FM999999999.00') as total_paid"),
            DB::raw("TO_CHAR(SUM(COALESCE(bt.gross_amount, 0) - sb.absolute_discount - COALESCE(bp.total_paid, 0)), 'FM999999999.00') as total_due")
        )
        ->where('sc.cid', $cid)
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw('SUM(COALESCE(bt.gross_amount, 0) - sb.absolute_discount - COALESCE(bp.total_paid, 0)) > 0')
