...
public function b2cSalesReport(Request $request)
{
    ...
    $data = DB::table('sales_items as si')
        ...
        ->join('products as p', 'si.pid', '=', 'p.id')
        ->join('sales_clients as sc', 'sb.scid', '=', 'sc.id')
        ->where('sb.uid', auth()->id())  // Only current user's bills
        ->where(function ($q) {
        ...
        ->select(
            ...
            'p.hscode',
            'si.gst as gst_rate',
            DB::raw('ROUND(SUM(si.quantity * si.s_price * (100 - COALESCE(si.dis, 0)) / 100), 2) as total_taxable'),
            DB::raw('ROUND(SUM(si.quantity * si.s_price * (100 - COALESCE(si.dis, 0)) / 100 * (si.gst / 100)), 2) as total_gst')
        )
        ->groupBy('p.id', 'p.name', 'p.hscode', 'si.gst')
        ->orderBy('p.name')
        ->get();
