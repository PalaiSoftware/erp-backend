...
public function getCustomersWithDues($cid)
{
    ...
    // Subquery to calculate correct bill_total (item sum after dis/gst - absolute_discount once per bill)
    // and bill_paid (initial paid_amount)
    ...
    // Main aggregation per customer, including extra payments
    $customersWithDues = DB::table('sales_clients as sc')
        ->leftJoinSub($billAggregates, 'ba', function ($join) {
            $join->on('sc.id', '=', 'ba.customer_id');
        })
        ->leftJoin('customer_bill_payments as cp', 'sc.id', '=', 'cp.customer_id')
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
