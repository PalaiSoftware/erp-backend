...
public function b2cSalesReport(Request $request)
{
    ...
    $companyState = DB::table('clients')->where('id', $cid)->value('state');

    // Group by product + GST rate → one row per product
    $report = DB::table('sales_items as si')
        ...
        ->where('u.cid', $cid)
        ->where(function ($q) {
            $q->whereNull('sc.gstin')->orWhere('sc.gstin', '');
        })
        ...
        ->select(
            ...
            'p.serial_number',
            'si.gst as gst_rate',
            // Detect intra & inter state sales
            DB::raw('MAX(CASE WHEN sc.state = ? THEN 1 ELSE 0 END) as has_intra'),
            DB::raw('MAX(CASE WHEN sc.state != ? OR sc.state IS NULL THEN 1 ELSE 0 END) as has_inter'),
            // Amounts
        ...
        ->groupBy('p.id', 'p.name', 'p.hscode', 'p.serial_number', 'si.gst')
        ->setBindings([$companyState, $companyState])
        ->orderBy('p.name')
        ->get();
    ...
    $finalReport = $report->map(function ($row) {
        ...
        $cgst = $row->has_intra ? number_format($gst / 2, 1) : '0.0';
        $sgst = $row->has_intra ? number_format($gst / 2, 1) : '0.0';
        $igst = $row->has_inter ? number_format($gst, 1) : '0.0';

        return (object) [
            'item_name'      => $row->item_name . ($row->serial_number ? " ({$row->serial_number})" : ''),
