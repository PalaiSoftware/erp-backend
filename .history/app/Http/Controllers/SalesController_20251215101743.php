...
public function getCustomersWithDues($cid)
{
    ...
    // First, calculate correct per-bill totals (item sum - absolute_discount once per bill)
    $perBillTotals = DB::table('sales_bills as sb')
    ...
    // Now aggregate per customer, including extra payments
    $customersWithDues = DB::table('sales_clients as sc')
        ->leftJoinSub($perBillTotals, 'bt', function ($join) {
            $join->on('sc.id', '=', 'bt.customer_id');
        })
        ->leftJoin('customer_bill_payments as cp', 'sc.id', '=', 'cp.customer_id')
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
            DB::raw("TO_CHAR(COALESCE(SUM(bt.bill_total), 0), 'FM999999999.00') as total_purchase"),
            DB::raw("TO_CHAR(COALESCE(SUM(bt.bill_paid), 0) + COALESCE(SUM(cp.paid_amount), 0), 'FM999999999.00') as total_paid"),
            DB::raw("TO_CHAR(GREATEST(0, COALESCE(SUM(bt.bill_total), 0) - COALESCE(SUM(bt.bill_paid), 0) - COALESCE(SUM(cp.paid_amount), 0)), 'FM999999999.00') as total_due")
        )
        ->where('sc.cid', $cid)
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw('COALESCE(SUM(bt.bill_total), 0) - COALESCE(SUM(bt.bill_paid), 0) - COALESCE(SUM(cp.paid_amount), 0) > 0')
        ->orderByDesc(DB::raw('GREATEST(0, COALESCE(SUM(bt.bill_total), 0) - COALESCE(SUM(bt.bill_paid), 0) - COALESCE(SUM(cp.paid_amount), 0))'))
        ->get();
