...
public function setPriceByCustomerType(Request $request, $productId)
{
    ...
    $validated = $request->validate([
        ...
        'unit_id'          => 'required|integer|exists:units,id',   // ← NEW
    ]);

    $product = Product::findOrFail($productId);

    if ($product->cid !== $user->cid) {
        return response()->json(['message' => 'Unauthorized product'], 403);
    }

    $price = $validated['selling_price'];
    $inputUnit = $validated['unit_id'];

    // If product has secondary unit → convert to secondary (piece)
    if ($product->s_unit && $product->c_factor > 1) {
        if ($inputUnit == $product->p_unit) {
            $price = $price / $product->c_factor; // packet → piece
        }
        // else already per piece → keep
    }

    $record = \App\Models\ProductPriceByType::updateOrCreate(
        [
    ...
    return response()->json([
        'message' => 'Price set successfully (normalized to smallest unit)',
        'selling_price_stored' => $price,
        'data' => $record
    ]);
}
...
public function getUnitsByProductId(Request $request, $product_id)
{
    ...
    try {
        ...
        // ────────────────────────────────────────────────
        // NEW LOGIC: Normalize all prices to SECONDARY unit first
        // (smallest unit = most consistent base)
        // ────────────────────────────────────────────────
        $purchaseSecondary = $basePurchasePrice;
        $saleSecondary     = $finalSalePrice;

        // If prices are stored per PRIMARY unit → convert to secondary
        if ($priceUnitId && $priceUnitId == $product->p_unit && $cFactor > 1) {
            $purchaseSecondary = $basePurchasePrice / $cFactor;
            $saleSecondary     = $finalSalePrice     / $cFactor;
        }
        // If already per secondary or no info → keep as is

        // Now prepare display prices for both units
        $unitPricing = [];

        // Primary unit (e.g. Packet)
        if ($product->p_unit && $units->has($product->p_unit)) {
            $purchasePrimary = $purchaseSecondary * $cFactor;
            $salePrimary     = $saleSecondary     * $cFactor;

            $unitPricing[] = [
                'unit_id'           => $product->p_unit,
                'unit_name'         => $units[$product->p_unit],
                'purchase_price'    => round($purchasePrimary, 2),
                'sale_price'        => round($salePrimary, 2),
                'gst'               => $baseGst,
                'profit_percentage' => $profitPercentage,
            ];
        }

        // Secondary unit (e.g. Piece)
        if ($product->s_unit && $units->has($product->s_unit)) {
            $unitPricing[] = [
                'unit_id'           => $product->s_unit,
        ...
        // ────────────────────────────────────────────────
        // Optional: Add debug log to verify numbers
        // Remove or comment out in production
        // ────────────────────────────────────────────────
        \Log::info('getUnitsByProductId - Pricing debug', [
            'product_id'       => $product_id,
            'c_factor'         => $cFactor,
            'price_unit_id'    => $priceUnitId,
            'stored_purchase'  => $basePurchasePrice,
            'stored_sale'      => $finalSalePrice,
            'secondary_sale'   => $saleSecondary,
            'primary_sale'     => isset($salePrimary) ? $salePrimary : null,
            'unit_pricing'     => $unitPricing,
        ]);
