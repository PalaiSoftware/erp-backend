...
public function setPriceByCustomerType(Request $request, $productId)
    {
        ...
        $validated = $request->validate([
            ...
            'unit_id'          => 'required|integer|exists:units,id',  // ← NEW: mandatory
        ]);
        ...
        $productInfo = \App\Models\ProductInfo::where('pid', $productId)
            ->where('cid', $user->cid)
            ->first();

        if (!$productInfo) {
            return response()->json([
                'message' => 'Cannot set price: Product must be purchased at least once first'
            ], 422);
        }

        $baseUnitId    = $productInfo->unit_id;
        $inputUnitId   = $validated['unit_id'];
        $inputPrice    = $validated['selling_price'];
        $convertedPrice = $inputPrice;

        // If product has both units → convert input price to secondary unit (piece)
        if ($product->s_unit && $product->c_factor > 1) {
            if ($inputUnitId == $product->p_unit) {
                // Input was per packet → convert to per piece
                $convertedPrice = $inputPrice / $product->c_factor;
            } elseif ($inputUnitId != $product->s_unit) {
                return response()->json(['message' => 'Invalid unit for this product'], 422);
            }
            // else: already per piece → no conversion needed
        }
        ...
        return response()->json([
            'message'          => 'Selling price set successfully (normalized to smallest unit)',
            'product_id'       => $productId,
            'customer_type_id' => $validated['customer_type_id'],
            'unit_used'        => $inputUnitId,
            'price_input'      => $inputPrice,
            'price_stored'     => $convertedPrice,   // this is now per piece
            'data'             => $price
        ], 200);
    }

    /**
     * Get units and prices — now treats stored selling price as per SECONDARY unit
     */
    public function getUnitsByProductId(Request $request, $product_id)
    {
        ...
        try {
            ...
            // ──────────────────────────────────────────────────────────────
            // CORE FIX: Treat stored sale price as PER SECONDARY UNIT (piece)
            // ──────────────────────────────────────────────────────────────
            $purchaseSecondary = $basePurchasePrice;
            $saleSecondary     = $finalSalePrice;

            // If stored per PRIMARY unit → convert DOWN to secondary
            ...
            if ($product->p_unit && $units->has($product->p_unit)) {
                $unitPricing[] = [
                    ...
                    'sale_price'        => round($saleSecondary     * $cFactor, 2),
                    'gst'               => $baseGst,
