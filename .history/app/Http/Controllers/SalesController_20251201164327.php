...
public function b2cSalesReport(Request $request)
{
    ...
    $data = DB::table('sales_items as si')
        ...
        ->join('products as p', 'si.pid', '=', 'p.id')
        ->join('sales_clients as sc', 'sb.scid', '=', 'sc.id')
        ->join('users as u', 'sb.uid', '=', 'u.id')  // Join user to get cid
        ->where('u.cid', $cid)  // This is correct — user created the bill
        ->where(function ($q) {
        ...
        ->select(
            ...
            DB::raw('ROUND(SUM(si.quantity * si.s_price * (1 - COALESCE(si.dis, 0)/100) * (si.gst / 100)), 2) as total_gst')
        )
        ->groupBy('p.id', 'p.name', 'p.hscode', 'si.gst')
        ->orderBy('p.name')
        ->get();
    ...
    return response()->streamDownload(function () use ($report, $request) {
        ...
        $sheet->setCellValue('A1', 'B2C Sales Report (Unregistered Customers) - GSTR-1 Ready');
        ...
        $sheet->getStyle('A4:H4')->getFill()
            ...
            ->getStartColor()->setARGB('FFE1F5FE');
