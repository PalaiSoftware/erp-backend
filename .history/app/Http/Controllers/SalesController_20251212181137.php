...
public function getCustomerDues(Request $request, $customer_id)
{
    ...
    // READ amount from GET, POST, JSON – works everywhere
    $amount = round((float)preg_replace('/[^0-9.-]/', '', $request->input('amount', $request->query('amount', 0))), 2);
    ...
    // RE-CALCULATE LEDGER (Always fresh & correct)
    ...
    foreach ($bills as $b) {
        ...
        // Apply later payments FIFO
        if ($remainingLaterPaid > 0) {
            $stillDue = $billTotal - $paid;
            if ($stillDue > 0) {
