...
public function b2cSalesReport(Request $request)
{
    ...
    $report = DB::table('sales_items as si')
        ...
        ->select(
            ...
            'p.serial_number',
            'si.gst as gst_rate',
            // Detect if this product was ever sold with CGST+SGST
            DB::raw('MAX(si.cgst) as max_cgst'),
            DB::raw('MAX(si.sgst) as max_sgst'),
            DB::raw('MAX(si.igst) as max_igst'),
            // Total amounts
    ...
    $finalReport = $report->map(function ($row) {
        ...
        $cgstRate = $hasCGSTSGST ? number_format($gstRate / 2, 1) : '0.0';
        $sgstRate = $hasCGSTSGST ? number_format($gstRate / 2, 1) : '0.0';
        $igstRate = $hasIGST ? number_format($gstRate, 1) : '0.0';
