...
public function setPriceByCustomerType(Request $request, $productId)
{
    ...
    $enteredPrice = $validated['selling_price']; // user thinks: per primary (packet)
    $storedPrice = $enteredPrice;

    // Convert to per secondary (piece) if secondary exists
    if ($product->s_unit && $product->c_factor > 1) {
        $storedPrice = $enteredPrice / $product->c_factor; // 7000 / 50 = 140
    }
    ...
    return response()->json([
        'message'              => 'Selling price saved (converted to per secondary unit)',
        'entered_per_primary'  => $enteredPrice,
        'stored_per_secondary' => $storedPrice,
        'primary_equivalent'   => $product->s_unit ? round($storedPrice * $product->c_factor, 2) : $storedPrice,
        'data'                 => $price
    ], 200);
}
...
public function getUnitsByProductId(Request $request, $product_id)
{
    ...
    try {
        ...
        $basePurchasePrice = $productInfo ? (float) $productInfo->purchase_price : 0.0;
        $baseGst           = $productInfo ? (float) $productInfo->gst : 0.0;
        $profitPercentage  = $productInfo ? (float) ($productInfo->profit_percentage ?? 0) : 0.0;
        ...
        $cFactor = max(1, (float) $product->c_factor);

        // ──────────────────────────────────────────────────────────────
        // Always treat stored selling price as PER SECONDARY UNIT (piece)
        // Purchase unit is irrelevant for selling price display
        // ──────────────────────────────────────────────────────────────
        ...
        if ($product->p_unit && $units->has($product->p_unit)) {
            $unitPricing[] = [
                ...
                'sale_price'        => round($salePerSecondary * $cFactor, 2),  // e.g. 140 × 50 = 7000
                'gst'               => $baseGst,
