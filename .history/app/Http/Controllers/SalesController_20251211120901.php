...
public function getCustomerDues(Request $request, $customer_id)
{
    ...
    // Accept new payment (even if no purchase today)
    $amount = round((float)$request->query('amount', 0), 2);
    $paymentDate = $request->query('date', now()->format('Y-m-d'));
    $mode = $request->query('mode', 'Cash');
    $note = $request->query('note', 'Payment received');
    ...
    // Get all bills (oldest first for FIFO)
    ...
    $totalAvailable = $bills->sum('paid_amount') + 
    ...
    foreach ($bills as $bill) {
        $billAmount = round($bill->gross - ($bill->absolute_discount ?? 0), 2);
        $totalBilled += $billAmount;

        $paid = min($totalAvailable, $billAmount);
        $totalAvailable -= $paid;
        $due = $billAmount - $paid;
        $totalPaid += $paid;
    ...
    $ledger = array_reverse($ledger); // Show newest first
    $finalDue = $totalBilled - $totalPaid;

    $history = DB::table('customer_payments')
        ->where('customer_id', $customer_id)
    ...
    return response()->json([
        ...
        'current_due'   => number_format(max(0, $finalDue), 2),
        'ledger'        => $ledger,
