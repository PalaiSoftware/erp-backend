...
public function getCustomerDues(Request $request, $customer_id)
{
    ...
    // SUPER SAFE amount reading – works with commas, spaces, string, etc.
    $rawAmount = $request->input('amount') ?? $request->query('amount', '0');
    $amount = round((float)preg_replace('/[^\d.]/', '', $rawAmount), 2);
    ...
    foreach ($bills as $b) {
        ...
        $paid = $b->initial_paid;
        if ($remaining > 0 && ($billTotal - $paid) > 0){
            $apply = min($remaining, $billTotal - $paid);
            $paid += $apply;
            $remaining -= $apply;
        }
