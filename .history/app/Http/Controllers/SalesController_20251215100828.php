...
public function getCustomersWithDues($cid)
{
    ...
    $customersWithDues = DB::table('sales_clients as sc')
        ->leftJoin('sales_bills as sb', 'sc.id', '=', 'sb.scid')
        ->leftJoin('sales_items as si', 'sb.id', '=', 'si.bid')
        ->leftJoin('customer_bill_payments as cp', 'sc.id', '=', 'cp.customer_id')
        ->select(
            'sc.id as customer_id',
            'sc.name as customer_name',
            DB::raw("TO_CHAR(COALESCE(SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100)) - SUM(COALESCE(sb.absolute_discount, 0)), 0), 'FM999999999.00') as total_purchase"),
            DB::raw("TO_CHAR(COALESCE(SUM(sb.paid_amount), 0) + COALESCE(SUM(cp.paid_amount), 0), 'FM999999999.00') as total_paid"),
            DB::raw("TO_CHAR(
                COALESCE(SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100)) - SUM(COALESCE(sb.absolute_discount, 0)) - SUM(COALESCE(sb.paid_amount, 0)), 0)
                - COALESCE(SUM(cp.paid_amount), 0)
            , 'FM999999999.00') as total_due")
        )
        ->where('sc.cid', $cid)
        ->where(function ($query) {
            $query->whereNotNull('sb.id')  // Ensure only customers with at least one bill
                  ->orWhereExists(function ($sub) {
                      $sub->select(DB::raw(1))
                          ->from('customer_bill_payments')
                          ->whereColumn('customer_id', 'sc.id');
                  });
        })
        ->groupBy('sc.id', 'sc.name')
        ->havingRaw(
            "COALESCE(SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100)) - SUM(COALESCE(sb.absolute_discount, 0)) - SUM(COALESCE(sb.paid_amount, 0)), 0) - COALESCE(SUM(cp.paid_amount), 0) > 0"
        )
        ->get();
