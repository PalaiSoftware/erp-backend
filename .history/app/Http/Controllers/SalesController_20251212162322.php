...
public function getCustomersWithDues($cid)
{
    ...
    foreach ($customers as $c) {
        $summary = $this->calculateCustomerDueSummary($c->id);
        if ((float)str_replace(',', '', $summary['current_due']) > 0) {
            $result[] = [
                'customer_id'    => $c->id,
                'customer_name'  => $c->name,
                'total_purchase' => $summary['total_billed'],
...
// ADD THIS METHOD ANYWHERE INSIDE THE CLASS (preferably near the top or bottom)
private function calculateCustomerDueSummary($customer_id)
{
    ...
    $totalPaid = DB::table('customer_bill_payments')
