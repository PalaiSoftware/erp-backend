...
public function getCustomersWithDues($cid)
{
    ...
    // First: Get all customers who have at least one bill (to ensure all with dues are included)
    $customersWithBills = DB::table('sales_clients as sc')
        ->join('sales_bills as sb', 'sc.id', '=', 'sb.scid')
        ->select('sc.id as customer_id', 'sc.name as customer_name')
        ->where('sc.cid', $cid)
        ->groupBy('sc.id', 'sc.name');

    // Subquery for correct per-bill totals
    $billAggregates = DB::table('sales_bills as sb')
        ->leftJoin('sales_items as si', 'sb.id', '=', 'si.bid')
        ->select(
            'sb.scid as customer_id',
            DB::raw('COALESCE(SUM(si.s_price * si.quantity * (1 - COALESCE(si.dis, 0)/100) * (1 + COALESCE(si.gst, 0)/100)), 0) - COALESCE(sb.absolute_discount, 0) as bill_total'),
            DB::raw('COALESCE(sb.paid_amount, 0) as bill_paid')
        )
        ->groupBy('sb.id', 'sb.scid', 'sb.absolute_discount', 'sb.paid_amount');

    // Final query: Start from customers with bills, left join aggregates and payments
    $customersWithDues = DB::tableSub($customersWithBills, 'cwb')
        ->leftJoinSub($billAggregates, 'ba', function ($join) {
            $join->on('cwb.customer_id', '=', 'ba.customer_id');
        })
        ->leftJoin('customer_bill_payments as cp', 'cwb.customer_id', '=', 'cp.customer_id')
        ->select(
            'cwb.customer_id',
            'cwb.customer_name',
            DB::raw("TO_CHAR(COALESCE(SUM(ba.bill_total), 0), 'FM999999999.00') as total_purchase"),
            DB::raw("TO_CHAR(COALESCE(SUM(ba.bill_paid), 0) + COALESCE(SUM(cp.paid_amount), 0), 'FM999999999.00') as total_paid"),
            DB::raw("TO_CHAR(GREATEST(0, COALESCE(SUM(ba.bill_total), 0) - COALESCE(SUM(ba.bill_paid), 0) - COALESCE(SUM(cp.paid_amount), 0)), 'FM999999999.00') as total_due")
        )
        ->groupBy('cwb.customer_id', 'cwb.customer_name')
        ->havingRaw('COALESCE(SUM(ba.bill_total), 0) - COALESCE(SUM(ba.bill_paid), 0) - COALESCE(SUM(cp.paid_amount), 0) > 0')
        ->orderByRaw('GREATEST(0, COALESCE(SUM(ba.bill_total), 0) - COALESCE(SUM(ba.bill_paid), 0) - COALESCE(SUM(cp.paid_amount), 0)) DESC')
        ->get();
