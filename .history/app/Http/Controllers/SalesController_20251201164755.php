...
public function b2cSalesReport(Request $request)
{
    ...
    // THIS IS THE ONLY QUERY THAT WORKS 100% WITH YOUR CURRENT DB
    $data = DB::table('sales_items as si')
        ->join('sales_bills as sb', 'si.bid', '=', 'sb.id')
        ->join('products as p', 'si.pid', '=', 'p.id')
        ->join('sales_clients as sc', 'sb.scid', '=', 'sc.id')
        ->where('sb.uid', $user->id)  // Only bills created by current logged-in user
        ->where(function ($q) {
            $q->whereNull('sc.gst_no')
              ->orWhere('sc.gst_no', '=', '')
              ->orWhere('sc.gst_no', '=', 'NULL')
              ->orWhereRaw("TRIM(sc.gst_no) = ''");
        })
        ...
        ->select(
            ...
            DB::raw('
                ROUND(
                    SUM(
                        si.quantity * si.s_price * (100 - COALESCE(si.dis, 0)) / 100
                    ), 2
                ) as total_taxable
            '),
            DB::raw('
                ROUND(
                    SUM(
                        si.quantity * si.s_price * (100 - COALESCE(si.dis, 0)) / 100 * (si.gst / 100)
                    ), 2
                ) as total_gst
            ')
        )
    ...
    if ($data->isEmpty()) {
        return response()->json(['message' => 'No B2C sales found for you in this period'], 404);
    }
